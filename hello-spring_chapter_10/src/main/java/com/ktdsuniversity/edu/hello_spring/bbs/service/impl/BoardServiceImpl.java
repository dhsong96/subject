...
public class BoardServiceImpl implements BoardService {
	...
	// Spring Bean : @Controller, @Service, @Repository의 부모 @Component까지 정의되어있는 곳만 쓸 수 있다.
...
//	private String baseDirectory;
	...
	public boolean creatNewBoard(WriteBoardVO writeBoardVO) {
		...
		int insertBoard = this.boardDao.insertNewBoard(writeBoardVO);
		return insertBoard > 0;
	}
	...
	public BoardVO updateOneBoard(int id, boolean isIncrease) {
		...
		if(boardVO == null) {
			throw new IllegalArgumentException("접근 잘못하셨어요~");
		}
		return boardVO;
	}
	...
	public boolean deleteOneBoard(int id) {
		...
		int deleteBoard = boardDao.deleteOneBoard(id);
